...
async def test_batch_generates_count_images_with_distinct_prompts(sessions_file, variables_file):
    ...
    combos = [
        ("de pie, frontal", {"pose": "de pie", "angle": "frontal", "action": "mirando"}),
        ("sentado, lateral", {"pose": "sentado", "angle": "lateral", "action": "mirando"}),
        ("acostado, cenital", {"pose": "acostado", "angle": "cenital", "action": "mirando"}),
    ]
...
async def test_batch_reuses_original_image(sessions_file, variables_file):
    ...
    combos = [
        ("a, b", {"pose": "a", "angle": "b", "action": "c"}),
        ("d, e", {"pose": "d", "angle": "e", "action": "f"}),
    ]
...
async def test_batch_continues_on_provider_error(sessions_file, variables_file):
    ...
    combos = [
        ("ok1, b", {"pose": "ok1", "angle": "b", "action": "c"}),
        ("fail, e", {"pose": "fail", "angle": "e", "action": "f"}),
        ("ok3, h", {"pose": "ok3", "angle": "h", "action": "i"}),
    ]
    with patch.object(bot, "generate_image", side_effect=_fake_gen) as mock_gen:
        with patch.object(bot, "process_image_result", new_callable=AsyncMock) as mock_res:
            with patch("variables_store.random_combination", side_effect=combos):
...
async def test_batch_continues_on_generate_exception(sessions_file, variables_file):
    ...
    combos = [
        ("ok1, b", {"pose": "ok1", "angle": "b", "action": "c"}),
        ("boom, e", {"pose": "boom", "angle": "e", "action": "f"}),
        ("ok3, h", {"pose": "ok3", "angle": "h", "action": "i"}),
    ]
    with patch.object(bot, "generate_image", side_effect=_fake_gen) as mock_gen:
        with patch.object(bot, "process_image_result", new_callable=AsyncMock) as mock_res:
...
async def test_batch_shuffles_and_retries_once_on_exhaustion(sessions_file, variables_file):
    ...
    with patch.object(bot, "generate_image", side_effect=_fake_gen) as mock_gen:
        with patch.object(bot, "process_image_result", new_callable=AsyncMock) as mock_res:
            with patch(
                "variables_store.random_combination",
                return_value=("de pie de frente", {"pose": "de pie", "angle": "de frente", "action": "mirando"}),
            ):
...
async def test_batch_text_mode_failure_notice(sessions_file, variables_file):
    ...
    combos = [
        ("fail, b", {"pose": "fail", "angle": "b", "action": "c"}),
        ("ok2, e", {"pose": "ok2", "angle": "e", "action": "f"}),
    ]
    with patch.object(bot, "generate_image", side_effect=_fake_gen):
        with patch.object(bot, "process_image_result", new_callable=AsyncMock) as mock_res:
            with patch("variables_store.random_combination", side_effect=combos):
...
async def test_batch_cancel_stops_after_completed(sessions_file, variables_file):
    ...
    combos = [
        ("a1, b1", {"pose": "a1", "angle": "b1", "action": "c1"}),
        ("a2, b2", {"pose": "a2", "angle": "b2", "action": "c2"}),
        ("a3, b3", {"pose": "a3", "angle": "b3", "action": "c3"}),
    ]
...
async def test_batch_regen_context_has_kie_provider(sessions_file, variables_file):
    ...
    with patch.object(bot, "generate_image", side_effect=_fake_gen) as mock_gen:
        with patch.object(bot, "process_image_result", side_effect=_fake_process):
            with patch(
                "variables_store.random_combination",
                return_value=("de pie, frontal", {"pose": "de pie", "angle": "frontal", "action": "mirando"}),
            ):
...
async def test_batch_passes_caption_prompt_to_result_senders(sessions_file, variables_file):
    ...
    combos = [
        ("a1, b", {"pose": "a1", "angle": "b", "action": "c"}),
        ("a2, e", {"pose": "a2", "angle": "e", "action": "f"}),
    ]
    with patch.object(bot, "generate_image", side_effect=_fake_gen):
        with patch.object(bot, "process_image_result", new_callable=AsyncMock) as mock_res:
            with patch("variables_store.random_combination", side_effect=combos):
...
async def test_batch_failure_notifies_on_generate_exception(sessions_file, variables_file):
    ...
    combos = [
        ("boom, b", {"pose": "boom", "angle": "b", "action": "c"}),
        ("ok2, e", {"pose": "ok2", "angle": "e", "action": "f"}),
    ]
    with patch.object(bot, "generate_image", side_effect=_fake_gen):
        with patch.object(bot, "process_image_result", new_callable=AsyncMock) as mock_res:
